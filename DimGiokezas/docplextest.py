...
def build_model(cvrp_prob, tlim):
    ...
    demand = [ vrp.get_demand(c) for c in vrp.customers() ] #TODO κατευθείαν
...
if __name__ == "__main__":
    fname = os.path.dirname(os.path.abspath(__file__)) + os.path.sep + "data.txt"
    if len(sys.argv) != 1:
        if len(sys.argv) != 2:
